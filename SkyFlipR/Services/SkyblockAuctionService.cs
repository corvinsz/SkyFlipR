...
public interface ISkyblockAuctionService
{
    IAsyncEnumerable<Auction> GetBuyItNowAuctionsAsync(string? apiKey = null, CancellationToken cancellation = default);
}

public class SkyblockAuctionService : ISkyblockAuctionService
{
            ...
            foreach (var auc in auctionsNode.EnumerateArray())
            {
                    ...
                    isBuyItNow.GetBoolean())
                {
                    yield return new Auction
                    {
                        ...
                        Tier = auc.GetProperty("tier").GetString(),
                        Category = auc.GetProperty("category").GetString(),
                        CleansedItemName = auc.GetProperty("item_name").GetString().CleanseItemName()
                    };
                }
            }
